...
class RawData:
  ...
  def RawData_Sourced(self, save= False):
    ...
    for h5file in GAIT_FILE_NAME_LIST: # Extract signal from H5 files
        ...
        try:
            with h5py.File(DATA, 'r') as file:
                TIME_S = ((file["SI-002189"]['Time'][:]) - (file["SI-002189"]['Time'][:])[0])/1e6 # timestamp converted to seconds
                ACC_VERTICAL_MS2 = file["SI-002189"]["Calibrated"]["Accelerometers"][:][:, 0] # m/s2
                ACC_ML_MS2 = file["SI-002189"]["Calibrated"]["Accelerometers"][:][:, 1] # m/s2
                ACC_AP_MS2 = file["SI-002189"]["Calibrated"]["Accelerometers"][:][:, 2] # m/s2
                GYR_VERTICAL_DEGS = file["SI-002189"]["Calibrated"]["Gyroscopes"][:][:, 0] * 57.2958 # rad/s converted to degree/s
                GYR_ML_DEGS = file["SI-002189"]["Calibrated"]["Gyroscopes"][:][:, 1] * 57.2958 # rad/s converted to degree/s
                GYR_AP_DEGS = file["SI-002189"]["Calibrated"]["Gyroscopes"][:][:, 2] * 57.2958 # rad/s converted to degree/s
    ...
    MEDICAL_LIST[0] = MEDICAL_LIST[0].sort_values(by= ["Subject ID"])
